    ...
    for (const ext of supportedFileFormats) {
      ...
      try {
        if (ext === 'js') {
          ...
          const fileContents = (await fsp.readFile(filePath)).toString()
          data = this.#stringToObject(fileContents, ext)
        }
      }
      catch (error) {
        throw new Error(
          `Configuration file is broken (${filePath})`, { cause: error }
        )
      }
  ...
  mergeDefaults(fromFullData, toPartialData) {
    ...
    for (const name in outputConfig) {
      ...
      if (
        ...
        outputConfig[name] = (Object.keys(outputValue).length === 0)
          /*
           * The default value is an empty object, which means
           * the object can contain anything
           */
          ? inputValue
          : this.mergeDefaults(outputValue, inputValue)
      }
      else {
