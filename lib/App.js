...
import dbDrivers from './db-drivers/index.js'
...
function getUrlRewriteRules(configPath)
{
	...
	try
	{
		if ($fs.lstatSync(file).isFile())
		{
			rules = $jsonfile.readFileSync(file, {stripComments : true})
		}
	} catch (e)
	{
	}
...
class App
{
	...
	constructor(path, develop)
	{
		...
		this.config.outputFilesDirectory =
			$os.tmpdir() + $path.sep
			+ 'nodejs-galaxia' + $path.sep
			+ 'public-' + ((develop === true) ? 'dev' : 'prod') + $path.sep
			+ this.config['pathComponents']
	...
	parseStaticFile(appID, request, response)
	{
		...
		let allowedFolders            = ['public', 'script', 'stylesheet']
		...
		let directFile = $path.resolve(
			$config['appsPath'] + '/' // todo this one seems to be wrong
			+ appID + ''
