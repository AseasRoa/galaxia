class Interface
{
	...
	deleteMany(filter) {
		...
		return new Promise((resolve) => {
			...
			this.currentCollection.deleteMany(filter).then(resolveCallback, rejectCallback)
		})
	}
	...
	deleteOne(filter) {
		...
		return new Promise((resolve) => {
			...
			this.currentCollection.deleteOne(filter).then(resolveCallback, rejectCallback)
		})
	}
	 ...
	 *
	 * @returns {Promise} - (resolve only) Returns true if dropped or the collection didn't exist,
	 * otherwise false.
	 */
	drop() {
		this.lastError = null
			...
			this.db.collection(
				...
				(result) => {
					...
					if (result instanceof Error) {
						...
						this.currentCollection.drop().then(resolveCallback, rejectCallback)
					}
				}
			)
		})
	...
	insertMany(arrayOfDocuments) {
		...
		return new Promise((resolve) => {
			...
			this.currentCollection.insertMany(
				arrayOfDocuments,
				{
	...
	insertOne(document) {
		...
		return new Promise((resolve) => {
			...
			this.currentCollection.insertOne(
				document,
				{
					forceServerObjectId : true,
					j                   : false
	...
	readMany(query, projection) {
		let result = this.currentCollection.find(
			query,
			{projection : projection}
		).sort(this._sort).limit(this._limit).toArray()
	...
	unsetMany(filter, fields) {
		...
		return new Promise((resolve) => {
			this.currentCollection.updateMany(
				filter,
				{'$unset' : fieldsToUnset}
			).then((result) => {
	...
	unsetOne(filter, fields) {
		...
		return new Promise((resolve) => {
			this.currentCollection.updateOne(
				filter,
				{'$unset' : fieldsToUnset}
			).then((result) => {
				resolve(result.result.nModified)
