...
// -1 = able to restart workers
// 0 = restarting workers
// positive value = counting workers until their number becomes what we want,
// and then the value is set to -1
let flagForked     = 0
...
class MainCluster
{
	...
	forkWorkers()
	{
		if (cluster.isMaster)
		{
			...
			let workersCountNow = workersKeys.length

			if (workersCountNow > 0)
			{
				if (flagForked !== -1)
				{
	...
	setupIPC()
	{
		...
		ipc.on('restart', (message, callback) => {
			...
			else
			{
				...
				// this timeout is mostly because the 'RESTARTING' letters are
				// sometimes shown after the following message in the console
				setTimeout(() => {
	...
	start()
	{
		...
		cluster.on('listening', (worker) => {
			...
			if (currentWorkersCount >= expectedWorkersCount)
			{
				...
				{
					// this timeout is mostly because the 'HTTP listen on...' letters are
					// sometimes shown before the following message in the console
					setTimeout(() => {
		...
		stdin.resume() // so the program will not close instantly
