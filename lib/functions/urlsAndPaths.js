 ...
 * The input path should start with the component name,
 * like this: /component/...
...
export function isUnprocessedStyleFile(relativePath, dirName) {
  ...
  return (
    pathParts.length === 3 // Must be like ['component', 'styles', 'file.js']
    && pathParts[1] === dirName
...
export function isClientPath(relativePath, dirName) {
  ...
  return (
    // Must be like ['componentName', 'clientDirName', 'RouterFile.js']
    pathParts.length >= 2
    && pathParts[1] === dirName
  )
}
 ...
 * /componentName/routerDirName/RouterFile.js
 *
 * The input path is allowed to contain forward or backward slashes,
 * and it's also allowed to start with or without a slash.
 *
...
export function isRouterFile(relativePath, dirName) {
  ...
  return (
    // Must be like ['componentName', 'routesDirName', 'RouterFile.js']
    pathParts.length >= 3
    && pathParts[1] === dirName
    && allowedExtensions.includes(parsedPath.ext)
  )
 ...
 * @returns {boolean}
 */
export function isClientCssFile(relativePath,) {
  const pathParts = (relativePath instanceof Array)
    ? relativePath
    : pathSplit(relativePath)
  /** @type {string[]} */
  ...
  return (
    // Must be like ['componentName', 'style.css']
    pathParts.length === 2
    && allowedExtensions.includes(parsedPath.ext)
  )
}
 ...
 * that contains /server, for example: /component/server/anything
 *
 * @param {string | string[]} relativePath
 * The input path should start with the component name,
 * like this: /component/...
...
export function isPubliclyAccessiblePath(relativePath, forbiddenDirNames) {
  const pathParts = (relativePath instanceof Array)
    ? relativePath
    : pathSplit(relativePath)
  const dirInComponent = pathParts[1] ?? ''

  return !(forbiddenDirNames.has(dirInComponent))
}
...
export function isJSFile(relativePath, dirNames) {
  ...
  return (dirNames.has(dirInComponent) && jsExtensions.includes(parsedPath.ext))
}
 ...
 * @returns {string}
 */
export function extractNodeModuleName(modulePath) {
  const allowedDirNames = ['@modules']
  const pathParts = (modulePath instanceof Array)
    ? modulePath
    : pathSplit(modulePath)
