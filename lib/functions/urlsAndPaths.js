...
export function isClientJSFile(relativePath, dirName) {
  ...
  return (
    isClientJSFile.allowedExtensions.has(parsedPath.ext)
    && isClientPath(relativePath, dirName)
  )
}
isClientJSFile.allowedExtensions
  = new Set(['.js', '.mjs', '.cjs'])
...
export function isRoutesFile(relativePath, dirName) {
  ...
  return (
    ...
    pathParts.length >= 3
    && pathParts[1] === dirName
    && isRoutesFile.allowedExtensions.has(parsedPath.ext)
  )
}
isRoutesFile.allowedExtensions
  = new Set(['.js', '.mjs', '.cjs'])
...
export function isJSFile(relativePath, dirNames) {
  ...
  return (
    ...
    && isJSFile.allowedExtensions.has(parsedPath.ext)
  )
}
isJSFile.allowedExtensions
  = new Set(['.js', '.mjs', '.cjs'])
