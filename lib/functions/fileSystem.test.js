// oxlint-disable vitest/no-conditional-expect, import/no-named-as-default-member
...
describe('File System Functions', () => {
  ...
  describe('Dirs', () => {
    ...
    test('isDir()', async() => {
      ...
      await expect(fileSystem.isDir(outputDir)).resolves.toBe(true)
      await expect(fileSystem.isDir(outputDir, 'not-there')).resolves.toBe(false)
      await expect(fileSystem.isDir(file)).resolves.toBe(false)
    })
    ...
    test('dirExists()', async() => {
      // Existing dir
      await expect(
        fileSystem.dirExists(outputDir)
      ).resolves.toBe(true)
      // Non-existent dir
      await expect(
        fileSystem.dirExists(outputDir, 'not-there')
      ).resolves.toBe(false)
    })
    ...
    test('ensureDir()', async() => {
      ...
      await expect(fileSystem.dirExists(dir)).resolves.toBe(false)

      fileSystem.ensureDirSync(dir)

      await expect(fileSystem.dirExists(dir)).resolves.toBe(true)
    })
    ...
    test('isDirEmpty()', async() => {
      await expect(fileSystem.isDirEmpty(outputDir)).resolves.toBe(true)

      await fileSystem.ensureDir(outputDir, 'dir')

      await expect(fileSystem.isDirEmpty(outputDir)).resolves.toBe(false)
    })
    ...
    test('isDirEmpty() extended scenario', async() => {
      ...
      await expect(fileSystem.isDirEmpty(file)).rejects.toThrow(Error)
    })
    ...
    test('dirStats()', async() => {
      ...
      // Non-existent dir
      await expect(
        fileSystem.dirStats(outputDir, 'not-there')
      ...
      await expect(fileSystem.dirStats(file)).rejects.toThrow(Error)
    })
    ...
    test('remove() directory', async() => {
      ...
      await expect(fileSystem.dirExists(outputDir)).resolves.toBe(true)

      await fileSystem.remove(outputDir)

      await expect(fileSystem.dirExists(outputDir)).resolves.toBe(false)
    })
  ...
  describe('Files', () => {
    test('fileExists()', async() => {
      ...
      await expect(fileSystem.fileExists(file)).resolves.toBe(false)

      await fileSystem.writeFile(file, 'data')

      await expect(fileSystem.fileExists(file)).resolves.toBe(true)
    })
    ...
    test('isFile()', async() => {
      ...
      await expect(
        fileSystem.isFile(outputDir)
      ).resolves.toBe(false)
      await expect(
        fileSystem.isFile(outputDir, 'not-there')
      ).resolves.toBe(false)
      await expect(
    ...
    test('fileSize()', async() => {
      ...
      await expect(fileSystem.fileSize(file)).resolves.toBe(9)

      // Directory, instead of a file
      await expect(
        () => fileSystem.fileSize(outputDir)
      ).rejects.toThrow(Error)
    })
    ...
    test('fileStats()', async() => {
      ...
      // Dir instead of file
      await expect(fileSystem.fileStats(outputDir)).rejects.toThrow(Error)
    })
  })
  ...
  describe('Path validation', () => {
    test('throws on invalid path (win32 only)', async() => {
      if (process.platform === 'win32') {
        ...
        await expect(
          fileSystem.writeFile(outputDir, ':')
        ).rejects.toThrow(Error)
      }
    })
