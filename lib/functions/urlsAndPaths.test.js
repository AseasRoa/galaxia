...
describe('URL Functions', () => {
  describe('URL functions', () => {
    test('normalizePathSlashes', () => {
      expect(
        normalizePathSlashes(String.raw`/a/b//c///d\e\\f\\\g`)
      ).toBe(
        '/a/b/c/d/e/f/g'
      )
    ...
    test('should normalize url path', () => {
      expect(
        normalizeURLPath(
          String.raw`/dir1\\dir2./dir3\../dir4/file.ext?key=value`
        )
      ).toBe(
    ...
    test('replacePathSeparators()', () => {
      const filePath = String.raw`C:\folder\file.ext`
  ...
  describe('Path Functions', () => {
    test('pathSplit()', () => {
      ...
      expect(fn(String.raw`\/dir///\file.ext`)).toStrictEqual(['dir', 'file.ext'])
    })
    ...
    test('isClientScript()', () => {
      ...
      // Try wrong paths
      expect(() => fn('/anything', '')).toThrow('Please, provide a dir name')
