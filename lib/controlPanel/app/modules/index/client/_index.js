...
function drawPerWorkerCharts(workersStats) {
  for (const stats of workersStats) {
...
setInterval(async() => {
  const stats = await workersStats()

  if (!stats) {
    return
  }

  drawSummarizedCharts(stats)

  if (stats.length > 1) {
    drawPerWorkerCharts(stats)
  }
}, 1000)
