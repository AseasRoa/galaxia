...
async function validateArguments(func, funcFile, funcArgs) {
  ...
  const result = docSchemaValidator.validateFunctionArguments(
    typesAst, funcArgs, false, true
  )

  if (!result.pass) {
    const error = new ValidationError()
    const match = /([\w']+)\(/u.exec(typesAst.lineAfterComment.trim()) ?? ''
    const fnName = (typeof match === 'string') ? match : match[1]

    error.stack = `ValidationError: ${result.message}\n    `
      + `at ${fnName} (file://${typesAst.file}:${typesAst.endLine + 1})`
