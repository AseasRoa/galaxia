 ...
 *
 *	http://russellhaering.com/ridiculously-fast-sprintf/
 */
 ...
 */
function sprintf(formatter)
{
	//if (formatter.indexOf("%") == -1) return formatter
			...
			if (formatter[i] === '%')
			{
				...
				switch (type)
				{
					case 's':
						...
						break
					case 'd':
						builder += `${formatter.slice(prev, i)}' + parseFloat(arguments[${arg}]) + '`
						prev = i + 2
						arg++
						break
					case 'j':
