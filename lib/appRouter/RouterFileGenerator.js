...
`
  ...
  return routesFetcher('', component, router, [], '${name}', arguments, ajaxVersion, '')
}
`
  }
  ...
  #generateMethod(name, isAsync) {
    ...
    return `
  ${async}${name}() {
    return routesFetcher(this.constructor.name, component, router, this.#constructorArgs, '${name}', arguments, ajaxVersion, this.#sessionKey)
  }
`
  }
