...
`
  ...
  return rpc('', module, router, [], '${name.replace(/'/ug, '\\\'')}', arguments, ajaxVersion, '')
}
`
  }
  ...
  #generateMethod(name, isAsync) {
    ...
    return `
  ${async}${name}() {
    return rpc(this.constructor.name, module, router, this.#constructorArgs, '${name.replace(/'/ug, '\\\'')}', arguments, ajaxVersion, this.#sessionKey)
  }
`
  }
