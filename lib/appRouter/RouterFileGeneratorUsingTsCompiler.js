...
class RouterFileGeneratorUsingTsCompiler {
   ...
   */
  async generate(sourceFileAbsPath, componentName, routerName, ajaxVersion) {
...
`
  ...
  return routesFetcher('', component, router, [], '${name.replace(/'/ug, '\\\'')}', arguments, ajaxVersion, '')
}
`
  }
  ...
  #generateMethod(name, isAsync) {
    ...
    return `
  ${async}${name}() {
    return routesFetcher(this.constructor.name, component, router, this.#constructorArgs, '${name.replace(/'/ug, '\\\'')}', arguments, ajaxVersion, this.#sessionKey)
  }
`
  }
