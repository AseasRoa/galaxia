...
class RouterProcessor {
  ...
  async process(layoutPath, pathParams, chunkParams) {
    ...
    try {
      ...
      return new Error(
        'Could not load'
        + ` "/${moduleName}/${routesDirName}/${routerFileName}"`
        + ` in order to load method "${ioMethodName}()".`
      )
    }

    const exports = (routerFileName === '' || routerFileName === 'index')
      ? moduleExport.default ?? moduleExport
      : moduleExport[routerFileName] ?? moduleExport.default ?? moduleExport
    const isRouterAClass = isClass(exports)
    ...
    if (isClass(exports)) {
      ...
      if (!exportedCallables) {
        ...
        const query = (isRouterAClass) ? queryParams.query?.[0] : []
        ...
        try {
          ...
          if (error instanceof Error) {
            error.message = `/${moduleName}/${routerFileName} > constructor: ${error.message}`
            // @ts-ignore
            error.isThrow = true
            returnedValue = error
          }
    ...
    if (!(returnedValue instanceof Error)) {
      ...
      if (typeof routerMethod !== 'function') {
        return new Error(`Router method <b>${ioMethodName}</b> for module <b>${moduleName}</b> does not exist!`)
      }

      // Call the function
      try {
        const methodArgs = this.#prepareMethodArguments(
          isHTML, isRouterAClass, queryParams, pathParams, request, response
        )
  ...
  async #addModulesAssets(
    moduleName, ioMethodName, locale, modulesAssets
  ) {
    modulesAssets.scripts.set(
      `${moduleName}.${ioMethodName}`,
      await this.#appFileManagers.getTagForJs(moduleName, ioMethodName)
    )
  ...
  async #processReturnedValue(
    ...
    if (
      ...
      try {
        ...
        console.error(`Could not generate view for /${moduleName}/${ioMethodName}`)
      ...
      if (isHTML) {
        await this.#addModulesAssets(
          moduleName, ioMethodName, words.locale, modulesAssets
        )
      }
    ...
    if (
      ...
      const html = (typeof funcOrString === 'string')
        ? funcOrString
        : await views.generateHtml(funcOrString, data)
