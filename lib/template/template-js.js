...
class Class
{
	...
	constructor(componentsDir, componentName, appConfig, appPaths) {
		...
		let componentsDirModified =
			    this.componentsDir.replace(new RegExp($path.sep.replace('\\', '\\\\'), 'g'), '-').
			    replace(/:/g, '-')
			...
			if (mtime) {
				...
				if (mtime !== false) {
					...
					if ($fs.existsSync(outputFileName)) {
						...
						let ajaxVersion = (typeof this.appConfig.ajax === 'object' && ('version' in this.appConfig.ajax))
	...
	render(componentName) {
		...
		this.renderDir(this.componentsDir, componentName, path_input, path_output)
	...
	renderDir(dirBase, componentName, pathInput, dirOutput) {
		...
		if ($fs.existsSync(dir)) {
			...
			for (var i in list) {
				...
				if (filename.substr(-3) === '.js') {
					...
					if (stat.isFile()) {
						this.renderFile(dirBase, componentName, pathInput, dirOutput, filename)
					}
				}
			}
		}
	...
	renderFile(dirBase, componentName, pathInput, dirOutput, filename) {
					...
					if (path[0] === '"' || path[0] === '\'' || path[0] === '`') {
						...
						if ($fs.existsSync(path_clear_full)) {
							...
							if (!(file_i in renderedFiles)) {
								...
								this.renderFile(dirBase, componentName, $path.dirname(path_clear), dirOutput, parsed.base)
							}
						}
						else {
