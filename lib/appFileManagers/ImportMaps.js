...
import { normalizePathSlashes } from '../functions/urlsAndPaths.js'
...
class ImportMaps {
  ...
  #extractModuleName(relativePath) {
    const secondSlashPos = normalizePathSlashes(relativePath).indexOf('/', 1)
  ...
  async #extractImports(code, relativePath) {
    const depth = stringCharacterOccurrences(
      normalizePathSlashes(relativePath),
      '/'
    ) - 1
    const imports = []
