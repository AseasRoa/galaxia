...
class FileManagerModules {
  ...
  async getModuleFile(moduleName) {
    ...
    let moduleNameClean = moduleName

    if (moduleName.startsWith('galaxia/')) {
      moduleNameClean = moduleName.substring(8)
      isAppModule = false
    }
    ...
    if (!record) {
      const path = await this.#getModuleFilePath(moduleNameClean, isAppModule)
      ...
      this.#modulesCache.set(moduleNameClean, record)
    }
