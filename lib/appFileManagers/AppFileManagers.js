...
class AppFileManagers extends EventEmitter {
  ...
  constructor(appConfig, appPaths) {
    ...
    this.#fileManagers = {
      ...
      clientJS: new FileManagerClientJS(this.appConfig, this.appPaths, this),
    ...
    this.#depsMap = [
      ...
      this.#fileManagers.clientJS.depsMap,
      this.#fileManagers.routesJS.depsMap,
    ]

    if (this.appConfig.development) {
      this.#watchForFileChanges(this.modulesDir)
  ...
  async getTagForCss(moduleName) {
    ...
    if (data === undefined) {
      ...
      if (await fileExists(outputFile)) {
        ...
        const url = `${moduleName}/${outputFileName}.css`
  ...
  async #pickOutputFile(moduleName, dirName, entryName = '') {
    ...
    const filesToQueryNoExt = [
      ...
      (additionalName)
        ? path.join(filesToQueryDir, `${moduleName}.${additionalName}`)
        : '',
      path.join(filesToQueryDir, `${moduleName}`),
    ]
   ...
   */
  #watchForFileChanges(modulesDir) {
