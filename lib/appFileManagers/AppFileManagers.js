...
class AppFileManagers extends EventEmitter {
  ...
  /**
   * Get an array of non-relative dependencies, imported in the files
   * of the app module.
   *
   * @param {string} moduleName
   * @returns {Promise<string[]>}
  ...
  async getImports(moduleName) {
    await this.#importMaps.ensureUniqueImportsAreLoaded(moduleName)

    return this.#importMaps.uniqueImports[moduleName] ?? []
  }
  ...
  async getModuleFile(modulePath) {
    ...
    return this.#fileManagers.modules.getModuleFile(modulePath)
  }
  ...
  async getTagForJs(moduleName, entryName = '') {
    ...
    if (data === undefined) {
      ...
      if (outputFile) {
        const size = await fileSize(outputFile)
        const ver = await this.getGlobalFilesVersion()
        const url = `${moduleName}/${this.appConfig.dirNames.client}/${path.basename(outputFile)}`
        const verUrl = `/${ver}/${url}`
   ...
   */
  async importRouterFile(moduleName, jsFileName) {
   ...
   * @returns {Promise<app.ImportsCacheItem>}
   */
  async importViewFile(moduleName, jsFileName) {
    return this.#fileManagers.routesJS.importViewFile(moduleName, jsFileName)
  }
  ...
  async #pickOutputFile(moduleName, dirName, entryName = '') {
    ...
    const filesToQueryDir = path.join(
      this.modulesDir, moduleName, dirName
    )
    const filesToQueryNoExt = [
      (additionalName) ? path.join(filesToQueryDir, `_${additionalName}`) : '',
      (additionalName) ? path.join(filesToQueryDir, additionalName) : '',
    ...
    if (inputFile) {
      ...
      outputFile = this.outputDir + inputFile.substring(this.modulesDir.length)
