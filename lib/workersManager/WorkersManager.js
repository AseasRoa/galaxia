...
/** @typedef {Map<number, ClusterWorker>} WorkersMap */
...
class WorkersManager {
   ...
   */
  async start() {
    const result = (this.#options.useCluster)
   ...
   * @returns {Promise<void>}
   */
  async startControlPanel() {
    const worker = new Worker({
      appPath: path.join(import.meta.dirname, '../controlPanel'),
      devMode: false,
  ...
  /**
   * @returns {Map<number, (ClusterWorker)>}
   */
  #getClusterWorkers() {
    /** @type {Map<number, (ClusterWorker)>} */
    const map = new Map()

    for (const id in cluster.workers) {
      /** @type {ClusterWorker | undefined} */
   ...
   * @returns {Promise<void>}
   */
  #killWorkers(workersMap, gracefully, reasonToKill, signal = 'SIGTERM') {
    for (const [id, worker] of workersMap) {
  ...
  #replaceWorkerIfTerminated() {
    ...
    const onExit = async(worker, code, signal) => {
      ...
      if (!reasonToKill) {
        /** @type {WorkersMap} */
        const workersToReplace = new Map().set(worker.id, worker)
  ...
  async #startAndSetupInitialWorkers(workersCount, workersTimeout) {
    const loaderFile = path.join(import.meta.dirname, 'register-hooks.js')

    // Change the default 'fork' behaviour for future fork() calls
    cluster.setupPrimary({
      exec: path.join(import.meta.dirname, '../worker/WorkerStarter.js'),
      args: [],
      silent: false,
      execArgv: [
        '--import',
  ...
  /**
   * @returns {Promise<void>}
   * @throws {Error} If the options suggest to start without cluster
   */
  async #startWithCluster() {
    ...
    this.#consoleMessage(
      `Cluster is spawning ${workersCount} worker${(workersCount === 1)
        ? ''
        : 's'}...`
    )
  ...
  #updateWorkersMap(workersToRemove, workersToAdd) {
    for (const [id, worker] of workersToRemove) {
      this.#workers.delete(worker.id)
    }

    for (const [id, worker] of workersToAdd) {
...
function killWorkers(workersMap, gracefully, signal = 'SIGTERM') {
  return new Promise((resolve) => {
    ...
    for (const [id, worker] of workersToKill) {
      if (worker.isDead()) {
        /*
