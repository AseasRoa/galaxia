...
class Model {
   ...
   * @throws {Error}
   */
  constructor(databaseName, collectionName, docSchema) {
    if (!connection) {
  ...
  /**
   * @param {string | Partial<SCHEMA>} index
   * @returns {Promise<boolean>}
   * @throws {import('mongodb').MongoServerError}
   */
  ...
  /**
   * @param {Record<keyof SCHEMA, number>} by
   * @returns {Query<SCHEMA>}
   */
  sort(by) {
  ...
  /**
   * @param {string} id
   * @returns {Promise<Partial<SCHEMA> | null>}
   */
  fetchById(id) {
  ...
  /**
   * @returns {Promise<Partial<SCHEMA>[]>}
   */
  fetchMany() {
