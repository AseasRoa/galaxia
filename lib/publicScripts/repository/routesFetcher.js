...
async function routesFetcher(
  ...
  if (className) headers['x-data-format'] = 'class'

  if (sessionKey) headers['x-session-key'] = sessionKey

  if (className) {
    headers['x-class-name'] = className
  }
  ...
  const response = await fetch(`/${component}/${router}/${methodName}`, {
    method: method,
    headers: headers,
    body: JSON.stringify(payload)
  })

  const responseType = (response.headers.get('x-response-type') ?? '').toLowerCase()
  ...
  if (response.status === 200) {
    switch (responseType) {
      ...
      case 'json': {
        ...
        return (text) ? routesFetcher.jsonParse(text) : null
      }
      case 'string': {
 ...
 * @returns {string}
 */
routesFetcher.randomString = function randomString(length) {
 ...
 * @returns {any}
 */
routesFetcher.jsonParse = function jsonParse(jsonString) {
...
if (window) window['routesFetcher'] = routesFetcher
