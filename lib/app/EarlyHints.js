...
import { fileStats, readFile } from '../functions/fileSystem.js'
import {
...
class EarlyHints {
  ...
  writeForHtmlResponse(response, ver, modulesAssets) {
    ...
    /** @type {string[]} */
    const links = []
    links.length = hintsCount
    let idx = 0

    for (const [key, asset] of modulesAssets.styles) {
     ...
     * ready to execute.
     */
    for (const [key, asset] of modulesAssets.scripts) {
  ...
  async #extractFileDeps(filePath, fileAbsPath) {
    /** @type {string[]} */
    const deps = []
    const buffer = await readFile(fileAbsPath)
    ...
    for (const match of matches) {
      ...
      const dep = (moduleName.startsWith('/@modules/'))
        ? moduleName
        : path.join(dirname, moduleName).replaceAll('\\', '/')
  ...
  async #processFile(fileRelPath, fileTmpAbsPath) {
    ...
    const stats = await fileStats(fileTmpAbsPath)
