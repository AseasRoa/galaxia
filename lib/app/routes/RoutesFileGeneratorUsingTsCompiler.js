...
`
    ...
    for (const [key, exp] of namedExports) {
      namedExportsString += exp
    }

    if (namedExportsString) output += namedExportsString

    if (defaultExports) output += defaultExports
...
`
  ...
  return rpc('', module, routes, [], '${name.replaceAll('\'', String.raw`\'`)}', arguments, ajaxVersion, '')
}
`
  }
  ...
  #generateMethod(name, isAsync) {
    ...
    return `
  ${async}${name}() {
    return rpc(this.constructor.name, module, routes, this.#constructorArgs, '${name.replaceAll('\'', String.raw`\'`)}', arguments, ajaxVersion, this.#sessionKey)
  }
`
  }
