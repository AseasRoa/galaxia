...
// Having this here seems to have about 5% better performance
// From "en-US,en" matches "en-US"
const preferredLocalePattern = /(?:^|,)\s*([a-zA-Z0-9\-]+)/u
...
class Words {
  ...
  /**
   * TODO Don't just get the preferred locale,
   * TODO but get the first locale that is supported
   *
  ...
  #getPreferredLocale(request) {
    ...
    // accept-language is for example: 'en-US,en'
    const header = request.headers['accept-language'] ?? ''
    const match = preferredLocalePattern.exec(header)

    let preferredLocale = ''

    if (match) {
      const fullLocale = (match[1] ?? '') // en-US
      const dashPos = fullLocale.indexOf('-')

      preferredLocale = (dashPos > -1)
        ? (fullLocale.slice(0, dashPos) ?? '')
        : (fullLocale ?? '')

      preferredLocale = preferredLocale.toLowerCase()
    }
