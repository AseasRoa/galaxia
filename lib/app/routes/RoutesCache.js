...
class RoutesCache {
   ...
   */
  constructor(request, classInstancesTimeoutMs = 60_000) {
...
function startGlobalPeriodicCleanup(periodMs, timeoutMs) {
  return setInterval(() => {
    ...
    for (const [key, item] of routesCache) {
      // Delete by timeout
      if (timeoutMs) {
...
function deleteFromCache(uniqueFileKey) {
  for (const [itemKey, item] of routesCache) {
    for (const [key, exportsMap] of item.exportsMap) {
