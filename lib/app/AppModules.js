...
import { ClientScripts } from './clientScripts/ClientScripts.js'
...
class AppModules {
  ...
  constructor(appConfig, appPaths, appFileManagers, earlyHints) {
    ...
    this.#publicScripts = new ClientScripts(!this.#appConfig.development)
  ...
  async processRequest(httpContext, explodedPathname) {
    ...
    const result = (request.method === 'GET' && !isXHR && isHTML)
      ...
      for (const attributes of tagContents) {
        if (attributes instanceof Object) {
          ...
          for (const name in attributes) {
            const value = (attributes[name] ?? '').replaceAll('"', '')
