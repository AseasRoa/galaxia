...
/**
 * @typedef FileManagers
 * @type {import('../FileManagers.js').FileManagers}
 */

// oxlint-disable-next-line unicorn/prefer-event-target
class FmServerScripts extends EventEmitter {
  /** @type {app.FullConfig} */
  #appConfig
  ...
  #supportedViewExtensions = new Set([
  ...
  async applyView(moduleName, viewName, data) {
    ...
    if (record === undefined) {
      ...
      for (const fileName of scanDir) {
        ...
        if (parsedPath.name === viewName) {
          if (this.#supportedViewExtensions.has(parsedPath.ext)) {
    ...
    return (record && record.template)
      ? applyTemplate(record.ext, record.template, data)
      : ''
  }
   ...
   * @throws {Error} An error if the module dir doesn't exist
   */
  async renderModule(moduleName) {
    ...
    if (!(await dirExists(moduleDir))) {
      throw new Error(`Module directory "${moduleDir}" not found.`)
    }
  }
