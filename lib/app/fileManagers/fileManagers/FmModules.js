...
class FmModules {
  ...
  async getModuleFile(modulePath) {
    ...
    if (modulePath.startsWith('galaxia/')) {
      modulePathClean = modulePath.slice(8)
      isAppModule = false
      // whitelist all from galaxia,
      // this covers files such as "galaxia/bundle.js.map"
      whitelisted = true
    }
  ...
  async #getNodeModuleData(rootDir, modulePath) {
     ...
     */
    const isModulePath = await isDir(nodeModulesDir, modulePath)
    const isFilePath = !isModulePath
    const moduleName = (isFilePath && indexOfFirstSlash !== -1)
      ? modulePath.slice(0, indexOfFirstSlash)
      : modulePath
    ...
    if (mainFile === undefined) {
      ...
      if (isModulePath) {
        ...
        if (!(await fileExists(mainFile))) {
          ...
          mainFile = join(
            nodeModulesDir,
            modulePath.slice(0, slashIndex),
            'dist',
            modulePath.slice(slashIndex + 1)
          )
