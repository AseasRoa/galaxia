...
import { FmClientScripts } from './fileManagers/FmClientScripts.js'
import { FmClientStyles } from './fileManagers/FmClientStyles.js'
import { FmModules } from './fileManagers/FmModules.js'
import { FmRoutesScripts } from './fileManagers/FmRoutesScripts.js'
import { FmServerScripts } from './fileManagers/FmServerScripts.js'
...
class FileManagers extends EventEmitter {
  /** @type {Watcher | null} */
  ...
  /**
   * @type {{
   *   clientStyles: FmClientStyles,
   *   clientScripts: FmClientScripts,
   *   routesScripts: FmRoutesScripts,
   *   serverScripts: FmServerScripts,
   *   modules: FmModules
   * }}
   */
  #fileManagers
  ...
  constructor(appConfig, appPaths) {
    ...
    this.#importMaps = new ImportMaps(
      this.#appConfig, this.#appPaths, this
    )

    this.#fileManagers = {
      clientStyles: new FmClientStyles(
        this.#appConfig, this.#appPaths, this
      ),
      clientScripts: new FmClientScripts(
        this.#appConfig, this.#appPaths, this, this.#importMaps
      ),
      routesScripts: new FmRoutesScripts(
        this.#appConfig, this.#appPaths, this
      ),
      serverScripts: new FmServerScripts(
        this.#appConfig, this.#appPaths, this
      ),
      modules: new FmModules(
    ...
    if (this.#appConfig.development) {
      ...
      this.#listenForserverScriptsFileChanges()
    }
  }
   ...
   */
  async ensureAllModulesAreRendered() {
    const dirScan = await readDir(this.#appPaths.modules)

    for (const dirName of dirScan) {
      const dirPath = join(this.#appPaths.modules, dirName)

      if (!(await isDir(dirPath))) {
        continue
      }

      await this.ensureModuleIsRendered(dirName)
    }
  }
   ...
   * @returns {Promise<string>}
   */
  buildImportMapScript(appModulesUsed) {
  ...
  async getTagForCss(moduleName) {
    ...
    if (data === undefined) {
      ...
      const { inputPathName, outputFileName }
        = this.#fileManagers.clientStyles.cssFilesManagerConfig
      const outputFile
        = this.#outputDir + sep
        + moduleName + sep
        + inputPathName + sep
      ...
      await this.#fileManagers.clientStyles.renderModule(moduleName)

      if (await fileExists(outputFile)) {
        const size = await fileSize(outputFile)
        const ver = await this.getGlobalFilesVersion()
        const url = `${moduleName}/${inputPathName}/${outputFileName}.css`
  ...
  async getTagForJs(moduleName, entryName = '') {
    ...
    if (data === undefined) {
      ...
      await this.#fileManagers.serverScripts.renderModule(moduleName)
      await this.#fileManagers.clientScripts.renderModule(moduleName)
      await this.#fileManagers.routesScripts.renderModule(moduleName)
   ...
   */
  importRoutesFile(moduleName, jsFileName) {
    return this.#fileManagers.routesScripts.importFile(moduleName, jsFileName)
  }
   ...
   * @returns {Promise<string>}
   */
  applyView(moduleName, viewName, data) {
    return this.#fileManagers.serverScripts.applyView(
      moduleName,
      viewName,
      data
    )
   ...
   * @returns {void}
   */
  #listenForserverScriptsFileChanges() {
    /**
    ...
    const managers = [
      this.#fileManagers.serverScripts,
      this.#fileManagers.routesScripts,
    ]
  ...
  async #pickOutputFile(moduleName, dirName, entryName = '') {
     ...
     */
    const additionalName = entryName.replaceAll(/[\/\\]/ug, '.')
    ...
    if (inputFile) {
      ...
      outputFile
        = this.#outputDir + inputFile.slice(this.#modulesDir.length)
