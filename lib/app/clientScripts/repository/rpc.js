...
async function rpc(
  ...
  try {
    response = await fetch(`/${moduleName}/${routesName}/${methodName}`, {
      method: method,
      headers: headers,
    ...
    // By default fetch() throws TypeError on network error,
    // but the message is diffent for each browser.
    // For that reason, throw TypeError with unified message here.
    throw new TypeError('Network Error', { cause: error })
  }
