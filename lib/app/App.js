...
class App {
  ...
  async parseRequest(exchange) {
    ...
    const isRouter = isRouterFile(pathnameSplitForFiles, dirNames.routes)
    ...
    if (nodeModulePathname) {
      await this.#staticFiles.parseNodeModule(exchange, nodeModulePathname)
    ...
    await this.#appModules.processRequest(exchange, pathnameSplit)
  }
  ...
  async start() {
    ...
    this.#staticFiles = new AppStaticFilesServer(
      this.config, this.paths, appFileManagers
    )
