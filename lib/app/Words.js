...
class Words {
   ...
   */
  constructor(appConfig, appPaths, componentName, request) {
  ...
  async getTranslation() {
    ...
    if (cache.get(host)?.get(componentName)?.get(locale) === undefined) {
      if (!cache.has(host)) {
        cache.set(host, new Map())
      }

      if (!cache.get(host)?.has(componentName)) {
        cache.get(host)?.set(componentName, new Map())
      }

      if (!cache.get(host)?.get(componentName)?.has(locale)) {
        const translation = await this.#readTranslation(componentName, locale)

        cache.get(host)?.get(componentName)?.set(locale, translation)
      }
    }

    // @ts-ignore
    return cache.get(host)?.get(componentName)?.get(locale)
  }
   ...
   */
  async #readTranslation(componentName, locale) {
    const fromJson = await this.#readTranslationFromJson(componentName, locale)
    const fromJs = await this.#readTranslationFromJs(componentName, locale)
   ...
   * @returns {Promise<TranslationMap>}
   */
  async #readTranslationFromJs(componentName, locale) {
    /** @type {TranslationMap} */
    let output = {}
    ...
    const fileWithoutExt = path.normalize(
      ...
      + `/${componentName}`
      + `/${this.appConfig.pathNames.serverFilesDirName}`
      + `/${this.appConfig.pathNames.i18nDirName}`
      + `/${locale}`
    )
   ...
   * @returns {Promise<TranslationMap>}
   */
  async #readTranslationFromJson(componentName, locale) {
    /** @type {TranslationMap} */
    let output = {}
