...
class Layout {
   ...
   * @param {RouterProcessor} routerProcessor
   */
  constructor(appConfig, appPaths, routerProcessor) {
  ...
  async makeLayoutHtml(explodedPathname, chunkParams) {
    ...
    const parts = this.#htmlToChunks(html, pathParams)

    let outputHtml = ''

    for (const part of parts) {
      outputHtml += (typeof part === 'function')
        ? await part(chunkParams)
        : part
    }
  ...
  async #getRawLayoutHtmlAndParams(explodedPathname, chunkParams) {
    ...
    const html = await this.#routerProcessor.process(
  ...
  #htmlToChunks = (string, pathParams) => {
    ...
    while (true) {
       ...
       */
      const chunkFn = async(chunkParams) => await this.#routerProcessor.process(
        layoutPath, pathParams, chunkParams
      )
