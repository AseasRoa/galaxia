 ...
 *
 * Why? Because the built-in "new URL()" is at least 2x slower
 * (as of Node.js version v21).
 */
class Url {
  #host = ''
  ...
  set host(host) {
    ...
    if (portIndex === -1) {
      ...
      this.#hostname = this.#host.slice(0, portIndex)
      this.#port = this.#host.slice(portIndex + 1)
    }
  ...
  #fromRequest(request) {
    ...
    this.pathname = (qIndex === -1)
      ? requestUrl
      : requestUrl.slice(0, qIndex)
    this.search = (qIndex === -1) ? '' : requestUrl.slice(qIndex)
