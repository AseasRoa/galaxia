...
import { ServerRequestsRateLimiter } from './ServerRequestsRateLimiter.js'

const requestsCounter = new ServerRequestsCounter(60)
...
class AppServer extends EventEmitter {
  ...
  /**
   * @type {Map<string | RegExp, ServerRequestsRateLimiter>}
   */
  ...
  constructor(appConfig) {
    ...
    if (this.appConfig.server.requestsRateLimits instanceof Array) {
      for (const rule of this.appConfig.server.requestsRateLimits) {
        if (
          rule.path instanceof RegExp
          || typeof rule.path === 'string'
        ) {
          const limiter = new ServerRequestsRateLimiter(
            rule.maxRequests,
            rule.secondsPeriod,
            rule.methods
          )
          ...
          throw new Error('requestsRateLimits.rule must be either RegExp or a string')
        }
      }
    }
  }
   ...
   * @param {function(HttpExchange):any} onRequest
  ...
  async start(onReady, onRequest) {
    return new Promise((resolve) => {
       ...
       */
      const onHttp1Request = (exchange) => {
        if (this.appConfig.server.redirectHttpToHttps) {
          const excludePaths
            = this.appConfig.server.redirectHttpToHttpsExcludePaths

          if (this.#redirectHttpToHttps(exchange, excludePaths)) {
            return
          }
        }

        this.#aggregatedRequestListener(exchange, onRequest)
      }

      /**
       * @param {HttpExchange} exchange
       */
      const onHttp2Request = (exchange) => {
        this.#aggregatedRequestListener(exchange, onRequest)
      }
   ...
   *
   * @param {HttpExchange} exchange
   * @param {function(HttpExchange):void} onRequest
   */
  #aggregatedRequestListener(exchange, onRequest) {
  ...
  #isRateLimited(request) {
    for (const [path, limiter] of this.#rateLimiters) {
      ...
      if (typeof path === 'string') {
        if (path === requestPath) match = true
      }
      else if (path instanceof RegExp) {
        if (path.test(requestPath)) match = true
      }

      if (match) {
        if (limiter.tooManyRequests(request)) {
          return true
        }
      }
    }
   ...
   */
  #redirectHttpToHttps(exchange, excludePaths) {
    const { request, response } = exchange

    if (
      (excludePaths instanceof Array)
