...
import { AppStaticFilesEarlyHints } from './AppStaticFilesEarlyHints.js'
...
class AppStaticFilesServer {
  ...
  /** @type {AppStaticFilesEarlyHints} */
  #appStaticFilesEarlyHints
  ...
  constructor(appConfig, appPaths, appFileManagers) {
    ...
    this.#appStaticFilesEarlyHints = new AppStaticFilesEarlyHints(
      this.appConfig,
      this.appPaths,
   ...
   */
  async parseNodeModule(exchange, modulePath) {
    try {
  ...
  async parseStaticFile(exchange, filePath, tryDirectFile = true) {
    ...
    if (!error && file && fileStats && fileStats.isFile()) {
      if (this.appConfig.server.earlyHints) {
        await this.#appStaticFilesEarlyHints.setEarlyHints(
          exchange.response,
          filePath,
          file
        )
