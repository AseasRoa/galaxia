...
class Class
{
	constructor(componentsDir, componentName, appName, configuration)
	{
		...
		let componentsDirModified =
				 ...
				 .replace(/:/g, "-")
		this.outputFilesDirectory = __tmpdir + $path.sep + componentsDirModified
...
`
	...
	module.exports = IO`
		...
		//log(argumentTypes)
		var file_o = this.outputFilesDirectory + $path.sep + componentName + "-arguments.json"
		$fs.writeFileSync(file_o, JSON.stringify(argumentTypes, null, 3))

		// write the output file
		var file_o = this.outputFilesDirectory + $path.sep + componentName + "-io.js"

		$fs.writeFileSync(file_o, contents)

		let codeForArguments = ""
		...
		{
			let module1 = Synchronator.require(file)

			module1.then((ioModule) => {

				if (typeof ioModule === "function")
				{
					// I use try catch here because this is not a normal way to call the class and errors are possible
					try
					{
						ioModule = new ioModule("Building the client io file")
					} catch (e)
					{
					...
					let methodNames = Object.getOwnPropertyNames(Object.getPrototypeOf(ioModule))

					// remove system method names from the list
					let tmp = []
					...
					for (let i in methodNames)
					{
						...
						if (methodName !== "constructor" && methodName !== "__autorun")
						{
							tmp.push(methodName)
						}
					}
					...
					let contents =
							 `
	let componentName = "${componentName}"
	let methodNames   = ${JSON.stringify(methodNames)}
	let IO = class
	{
		constructor() {
			this.__componentName = componentName
	...
	{
		let methodName = methodNames[i]
		IO.prototype[methodName] = function*() {return requireAjaxRequest(componentName, methodName, arguments, this.__args, this.__sess)}
	}
...
`
					...
					for (let i in methodNames)
					{
						...
						contents += `\t\t"${methodName}" : function*() {return ajax("${componentName}", "${methodName}", arguments)},\n`
					}

					contents += "\t}\n"
	*/
					contents = `require.register("${file2}", function*(module, exports, require) {` + contents + `\r\n})`

					contents = Synchronator.transform(contents).code

					// write the output file
					let file_o = this.outputFilesDirectory + $path.sep + componentName + "-io.js"
